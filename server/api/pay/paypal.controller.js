...
var OrderCtrl = /** @class */ (function (_super) {
    ...
    function OrderCtrl() {
        ...
        _this.PayPal = function (req, res, next) {
            var domain = process.env.DOMAIN || 'http://localhost:4200';
            var items = [];
            var subtotal = 0;
            var discount = 0;
            var data = JSON.parse(req.query.data);
            var options = JSON.parse(req.query.options);
            var total = 0; //data.price * data.quantity;
            ...
            var payReq = {
                'intent': 'sale',
                'redirect_urls': {
                    'return_url': domain + '/api/pay/process',
                    'cancel_url': domain + '/api/pay/cancel'
                },
                'payer': {
            ...
            paypal.payment.create(payReq, function (error, payment) {
                if (error) {
                    ...
                    res.redirect(domain + '/admin/my-orders?id=' + id + '&msg=' + msg);
                }
                else {
        ...
        _this.process = function (req, res) {
            ...
            paypal.payment.execute(paymentId, payerId, function (error, payment) {
                if (error) {
                    ...
                    res.redirect(domain + '/admin/my-orders?id=' + paymentId + '&msg=' + string);
                }
                else {
                    var mailParams_1 = {
                        id: payment.id,
                    ...
                    if (payment.state === 'approved') {
                        order_model_1.default.findOneAndUpdate({ 'payment.id': payment.id }, { "status": 'Paid', "payment.state": 'Paid' }, { upsert: false, setDefaultsOnInsert: true, runValidators: true }).exec()
                            .then(function (doc) {
                            vm.email(config.orderPlacedEmail(mailParams_1));
        ...
        _this.cancel = function (req, res) {
            ...
            res.redirect(domain + '/admin/my-orders?msg=' + string);
        };
        return _this;
    }
    return OrderCtrl;
